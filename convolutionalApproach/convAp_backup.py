...
# Increase time step for faster simulation (was 0.01)
...
if options.noise: 
    parameters = {
        'Rtarget':  0.005,      # Increased from 0.0025 for better activation
        'lambda_w': 0.00004,    # Increased from 0.00002 for faster learning
        'lambda_d': 0.0002,     # Increased from 0.0001 for faster learning
        'STDP_w':   0.02,       # Increased from 0.01 for stronger weight changes
        'STDP_d':   2.0         # Increased from 1.0 for stronger delay changes
    }
else:
    parameters = {
        'Rtarget':  0.006,      # Increased from 0.003 for better activation
        'lambda_w': 0.00006,    # Increased from 0.00003 for faster learning
        'lambda_d': 0.0012,     # Increased from 0.0006 for faster learning
        'STDP_w':   0.02,       # Increased from 0.01 for stronger weight changes
        'STDP_d':   2.0         # Increased from 1.0 for stronger delay changes
    }
...
# Generate patterns from audio files
audio_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "audio")
...
patterns = generate_audio_spike_patterns(
    ...
    specific_files=specific_audio_files
)
...
for conv_in, conv_out in it.permutations(convolutions,2):
    in2out = sim.Projection(
        ...
        synapse_type = sim.StaticSynapse(
            weight=50,
            delay=0.1  # Increased from 0.01 to avoid delay being less than resolution
        ),
...
if options.metrics:
    ...
    for i in range(NB_CONV_LAYERS):
        pattern_per_conv[i] = i % max(1, len(patterns) - 1)  # Cycle through available patterns, exclude -1
